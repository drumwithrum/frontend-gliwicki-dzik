class Auth {
  public static get isAuthorized(): boolean {
    return Boolean(this.token) && Boolean(this.userId);
  }

  public static deauthorize() {
    this.removeToken();
    this.removeUserId();
  }

  public static saveToken(token: string) {
    return Boolean(window.localStorage.setItem('gwdk-token', token));
  }
  public static saveUserId(id: string | number) {
    return Boolean(window.localStorage.setItem('gwdk-user-id', `${id}`));
  }

  public static removeToken() {
    return Boolean(window.localStorage.removeItem('gwdk-token'));
  }
  public static removeUserId() {
    return Boolean(window.localStorage.removeItem('gwdk-user-id'));
  }

  public static get token(): string | null {
    return window.localStorage.getItem('gwdk-token') || null;
  }
  public static get userId(): string | null {
    return window.localStorage.getItem('gwdk-user-id') || null;
  }
}

export default Auth;
